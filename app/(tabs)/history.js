...
export default function MedicineHistoryScreen() {
  ...
  const { medicines } = useAppContext(); // might use context if needed, but history is from storage
  const [history, setHistory] = useState([]);
  const fadeAnim = useRef(new Animated.Value(0)).current;

  // Realistically we want to reload this when it gets focused
  useEffect(() => {
    (async () => {
  ...
  }, [medicines]); // Re-fetch if medicines state changes (e.g. user toggles)
  ...
  return (
    <Animated.View style={{ flex: 1, backgroundColor: colors.bg, opacity: fadeAnim }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 30 }} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity 
          style={[styles.reportBtn, { backgroundColor: colors.primaryLight }]}
          onPress={() => router.push("/report")}
          activeOpacity={0.8}
        >
          <Text style={{ fontSize: 20, marginRight: 12 }}>📄</Text>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 14, fontFamily: 'Inter-ExtraBold', color: colors.primary }}>Generate Health Report</Text>
            <Text style={{ fontSize: 12, fontFamily: 'Inter-SemiBold', color: colors.primary, opacity: 0.8 }}>View and print your health summary</Text>
          </View>
        </TouchableOpacity>
